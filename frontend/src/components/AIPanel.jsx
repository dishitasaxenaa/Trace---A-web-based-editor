...
const AIPanel = ({ code, language }) => {
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      // Build conversation history — skip the welcome message and any loading placeholders
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: T.bg, borderLeft: `1px solid ${T.border}`, overflow: "hidden" }}>
      ...
      <div style={{ height: 44, background: T.surface, borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", padding: "0 14px", gap: 8, flexShrink: 0 }}>
        ...
        <div style={{ flex: 1 }} />
        <Badge color={T.purple}>Llama 3.3</Badge>
        <button
          onClick={() => setMessages(WELCOME)}
          title="Clear chat"
          style={{ background: "none", border: "none", color: T.textMut, cursor: "pointer", fontSize: 14, padding: "2px 6px", borderRadius: 4, transition: "color 0.15s" }}
      ...
      <div style={{ borderTop: `1px solid ${T.border}`, background: T.surface, flexShrink: 0, padding: "10px 12px" }}>
        <div
          style={{ display: "flex", gap: 8, alignItems: "flex-end", background: T.elevated, border: `1px solid ${T.border}`, borderRadius: 8, padding: "8px 10px", transition: "border-color 0.15s" }}
        ...
        >
          ...
          <button
            ...
            style={{
              background: input.trim() && !loading ? T.accent : T.accentDim,
              border: "none", borderRadius: 6, width: 30, height: 30, flexShrink: 0,
